...
export function InstaArchive({ data }: { data: Archive }) {
  const now = new Date();
  ...
  const media = useMemo(
    () => collection ? collection.media.filter((item) => item.year === year && item.month === month).sort((a, b) => b.date - a.date) : [],
      ...
      {!collection ? (
        ...
        <section className="member-gallery insta-gallery">
          <header className="member-gallery-head">
            <button onClick={() => setCollection(null)}>← ALL PROFILES</button>
            <div><span>INSTAGRAM POSTS / PROFILE</span><h2>{collection.name.toUpperCase()}</h2></div>
